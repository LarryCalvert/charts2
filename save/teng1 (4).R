...
# set our directory 
setwd("C:\\Users\\Lcalv\\Documents\\R\\teng4") #NOTE! Hard-coded for now!
#SRCDIR <- "test1" # below /use/documnet/r/
BASEDIR <- normalizePath(getwd())
LOGFILE <- paste0(BASEDIR, "\\teng4.log")
...
if (file.exists("detail.csv") == TRUE) {
  ...
  if (bubars == 0) {
    print(paste0("Data out of date too long - setting full tune"))
    fulltune <- TRUE 
  }    
  ...
  if (bubars > 0) {
    ...
    #fill in scr last bar's target and calc errs
    bupts <- (bubars-1)*ppb  
    ...
    for(i in seq(from = ppb-1, to = 0, by = -1)) {
      ...
      scr$error[nrow(scr)-i] = scr$target[nrow(scr)-i] - scr$predict[nrow(scr)-i]
      ###################################################
      # TODO make sderr calc in predict same as here
      dec <- 2
      sderr <- NA
      if (nrow(scr) >=TUNEBARS * ppb - ppb) {
        err <- tail(scr$error,TUNEBARS * ppb)
        sderr <- tail(round(STDE(err), digits=dec * 2),1)
      }
      scr$sderr[nrow(scr)-i] <- sderr
      ##########################################################
    }
    view(tail(scr,3*ppb))
    ...
    # do predicts
    print(paste0("Predicting ",bubars," bar(s) ",
                 symbol1, " ",
                 dt1," ",
                 pproc1," ",
                 method1              
                 ))
    # Need single complete def predict over nbr_bars here
    #scr2 <- pred_bars(
    #  symbol_list=tibble(symbol1),
    #  datatype_list=tibble(dt1),
    #  pproc_list=tibble(pproc1),
    #  method_list=tibble(method1),
    #  intl=INTL,
    #  nbr_bars=bubars
    #)
    # add rows scr2 to scr
    # truncate scr
    #dplyr::slice_tail( n=((TUNEBARS+1)*ppb))
    # add rows to detailw
    # when all symbols methods done DETAIL <- detailw
    # write detail.csv
  }
}

if (fulltune == TRUE) { # runs existing pred_bars for now
  print(paste0("Running fulltune"))
  #DETAIL <- pred_bars(
...
#plotly test
  ...
  zz <- TTR::ZigZag( zzin, change=0.8 ) # test
  ...
  fig <- fig %>% add_lines(x = df$date, y = df$hpred, name="hpred", line = list(color = 'purple', width = 0.75), inherit = F)
  fig <- fig %>% add_lines(x = df$date, y = df$lpred, name="lpred", line = list(color = 'purple', width = 0.75), inherit = F)
  fig <- fig %>% add_lines(x = df$date, y = df$cpred, name="cpred", line = list(color = 'red',    width = 1.25), inherit = F)
  fig <- fig %>% add_lines(x = df$date, y = df$mpred, name="mpred", line = list(color = 'green',  width = 1.25), inherit = F)
  ...
  # TODO now combine using subplot
  fig3 <- subplot(fig, fig2, heights = c(0.7,0.2), nrows=2,
                 shareX = TRUE, titleY = TRUE)
  fig3
  
  ########## Plot ###################################
  
  ####### Copy to server - Rpi for now ####################
  ...
  ############ git push
  dirsv <- getwd()
  setwd("C:\\Users\\Lcalv\\Documents\\R\\teng4\\charts")
