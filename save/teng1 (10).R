...
if (0) {
  ...
  date  <- request_data_l(msdataint=msdataint,type="date",bupts=0,len=clen)
  ...
  if (1) {
    ...
    in1 <- close
    p1 <- emd_predict2(in1,title = "July Wheat LB=30-06-01 Close")
  ################################################################
  }
...
DATA_CHANGED <- TRUE #TODO Will force a rescore -- tmp take out !
...
for(symrow in 1:nrow(PREDTAB)) { 
  ...
  ########### ERROR PREDICT #########################
  # test filtered
  f1 <- filter_func(in1,"zlma") #mf
  pred2a <- predict2(f1, ptype1="arma", ptype2="arma", chart = FALSE)
  #pred2a <- predict2(in1, ptype1="hybridcv", ptype2="hybridcv", chart = FALSE)
