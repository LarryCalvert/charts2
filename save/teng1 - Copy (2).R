...
#plotly test
  ...
  m1 <- function(parm1=NULL)
  {
    print(paste("This is M1 parm1=",parm1))
  }
  
  testp="test1"
  #TODI move adj parms here
  adj=0.10
  inc=0.01
  
  TRADE_METHOD_LIST <- tribble(
    ~tsys, ~bsc, ~func, ~desc,
    "M1", "Buy", m1(parm1=testp), "Buy - no protective stop move",
    #"M1", "Sell", 0.10, 0.01, # no protective stop move
    #"M1", "Comb", 0.10, 0.01, # no protective stop move
    #"M2", "Buy",  0.02, 0.01, # move protective stop
    #"M2", "Sell", 0.02, 0.01, # move protective stop
    #"M2", "Comb", 0.02, 0.01, # move protective stop        
  )
  ...
  TRADE_SUMMARY_LINES <- tribble(
    ~symbol, ~tsys, ~bsc, ~hadj, ~ladj, ~wins, ~losers, ~plmm, ~pldol,
  )
  
  TRADE_SUMMARY <- TRADE_SUMMARY_LINES  
  
  place_order <- function(orders=NULL,symbol=NULL,tm=NULL,hadj=NULL,ladj=NULL,bar=NULL,bors=NULL,ordt=NULL,nbr=NULL,price=NULL)
  {
    print(paste("TradeSim", tm$tsys, bar$date, "Place order", bors, ordt, price))
    orders <- orders %>%  
      add_row(
        symbol = symbol,
        tsys   = tm$tsys,
  ...
  ###################################################
  # trade simulator - called bar by bar
  ###################################################
  trade_simulator <- function(symbol=NULL,tm=NULL,hadj=NULL,ladj=NULL,pos=NULL,bar=NULL,pred=NULL)
  {
    orders <- ORDER
    ############################################################################
    if (pos == 0) { # not in a position place order for a possible entry
      if (tm$bsc == "Buy" || tm$bsc == "Comb") {
        #print(paste("TradeSim",tm$tsys,"Place order",tm$bsc,"stop at",pred$lpreda))
        orders = place_order(orders=orders,symbol=symbol,
                  tm=tm, hadj=hadj,ladj=ladj,
                  bar=bar, bors="Buy", ordt="Stop", price=pred$lpreda)
      }
      if (tm$bsc == "Sell" || tm$bsc == "Comb") {
        #print(paste("TradeSim",tm$tsys,"Place order",tm$bsc,"stop at",pred$lpreda))
        orders = place_order(orders,symbol=symbol,
                  tm=tm, hadj=hadj,ladj=ladj,
                  bar=bar, bors="Sell", ordt="Stop", price=pred$hpreda)
      }
    } # pos == 0
    ############################################################################   
  } 
  ...
  run_trade_system <- function(tm=NULL, df=NULL,hadj=NULL,ladj=NULL)
  {
    # now create adjusted predicts using hadj and ladj
    hpreda1 <- pred_adj(pred=df$hpred, adj=hadj, minmove=minmove)
    lpreda1 <- pred_adj(pred=df$lpred, adj=ladj, minmove=minmove) 
    
    position <- 0
    
    for (i in 1:nrow(df)) {
      bar <- df[i, ]  # Get the current row
      hpreda <- hpreda1[i]
      lpreda <- lpreda1[i]  
      cpreda <- df$cpred[i]
      mpreda <- df$mpred[i]
      #print(paste("RTS",bar$date,bar$high,bar$low,hpredb,lpredb,cpredb,mpredb))
      # gather up adjsued predicts
      pred <- tibble(hpreda,lpreda,cpreda,mpreda)      
      trade_simulator(symbol,tm=tm,hadj=hadj,ladj=ladj,pos=position,bar=bar,pred=pred)
      position = 1 # TEST ONLY TAKE OUT
    }
  }
    
  # cycles through all trade system adj and inc parms
  tune_trade_system <- function(tm=NULL)
  {
    for (hadj in seq(adj, -adj, by=-inc)) {
      for (ladj in seq(adj, -adj, by=-inc)) {
        print(paste("Trade system =",tm$tsys, tm$bsc, hadj, ladj))
        # now run trade system with these parms
        run_trade_system(tm=tm, df=df, hadj=hadj,ladj=ladj)
        #break # TAKE OUT
      }
      #break # TAKE OUT
    } 
  ...
  # Cycle through trade systems
  #view(df) # TMP TAKE OUT
  for (i in 1:nrow(TRADE_METHOD_LIST)) {
    TRADE_METHOD <- TRADE_METHOD_LIST[i, ]  # Get the current row
    tune_trade_system(TRADE_METHOD)
    #print(paste("Trade system =",TRADE_METHOD$tsys))
  }
  
  
  
  if (0) {
  # Needs to return single order
  # places order only if would have been placed
  
  do_order <- function(ORDERS=NULL,symbol=NULL,
                        method= NULL,
                        bors = NULL,
                        hadj = NULL,
                        ladj = NULL,
                        ...
                        pos  = NULL)
  {  
    #print(paste(symbol, trade_method, bors, hadj, ladj, rown))
    
    if (pos == 0) { # look for entry
      if (bors == "Buy") {
        if (df$low[rown] < preda$lpreda[rown] && df$close[rown] > preda$lpreda[rown]) {
          # Buy order filled
          pos = pos + 1
          ...
          ORDERS <- ORDER_LINE %>%  
            add_row(
              ...
              date = df$date[rown],
              bors = bors,
              price = preda$lpreda[rown],
              method = method,
              hadj = hadj,
              ladj = ladj
          )
          print(paste(symbol, df$date[rown], method, bors, "Stop at",
                preda$lpreda[rown], hadj, ladj, rown))
        }
      }
    }
    return(ORDERS)
  ...
  # TODO LOOP through methods and adjusts here = pred tune
  ############# TODO LOOP through methods and adjusts here  #############
  # For trade method
  for (i in seq(1:nrow(TRADE_METHOD_LIST)) ) {
    method <- TRADE_METHOD_LIST$method[[i]]
    bors <- TRADE_METHOD_LIST$bors[[i]]
    adjv <- TRADE_METHOD_LIST$adj[[i]]
    inc  <- TRADE_METHOD_LIST$inc[[i]]       
      
    # for hadj
    for (hadj in seq(adjv, -adjv, by = -inc)) {
      for (ladj in seq(adjv, -adjv, by = -inc)) { 
          
        # create adjusted HL predict lines
        hpreda <- pred_adj(pred=df$hpred, adj=hadj, minmove=minmove)
        lpreda <- pred_adj(pred=df$lpred, adj=ladj, minmove=minmove)          
        preda <- tibble(df$date,hpreda,lpreda)
          
        # for each bar
        pos <- 0
        ORDERS <- ORDER_LINES # start with empty orders
        for (rown in seq(1, nrow(df)-1)) { # per bar loop
          tmp_ORDER <- do_order(symbol=symbol,
                    method=method,
                    bors=bors,
                    hadj=hadj,
                    ladj=ladj,
                    ...
                    rown=rown,
                    pos=pos)
            if (! is.null(tmp_ORDER)) {
              ORDERS <- ORDERS %>% add_row(tmp_ORDER)
              
              #ORDER_FILLED <- do_fill(ORDER=tmp_ORDER,
              #      df=df,
              #      rown=rown,
              #      pos=pos)
            }
        ...
        #summarize_trades()
        # check for highets profit
        # check for lowest loss with profit
      }
    }
  }
} # end of trade sim
