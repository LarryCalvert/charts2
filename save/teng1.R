...
GITPUSH <- TRUE
if (OS == "Linux") GITPUSH <- FALSE # RPI for now

TUNE_TRADES <- TRUE    # Limited tune if FALSE
...
for(symrow in 1:nrow(symbol_list )) { # symbol loop
  ...
  cat(paste(symbol1,desc1,source))
  log4r::info(logger, paste(symbol1,desc1,source))
...
#        trades[nbrtrades,"riskdol"] <- (trades[nbrtrades,"riskmm"] / minmove) * dollars # risk in dollars
  ...
  tune_trade_system <- function(tsrow=NULL)
  {
    # trade control
    if (TUNE_TRADES == TRUE) {
      adj=0.40 # LRC std dev range +-0.30
      inc=0.01 # increment
    } else {
...
##################################################
  ...
  if (GITPUSH == TRUE) {
    ...
    setwd(paste0(dirsv,"/charts"))
  	newdir <- getwd()
	  cat(paste0("System=",system,"  ","directory=",newdir))
